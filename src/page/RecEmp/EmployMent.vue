...
<router-link to="/Home">首页</router-link> /招聘工作
					</div>
          <router-link
  ...
  export default {
    ...
    async getListCount () {
      try {
        const res = await this.$http.get(`http://localhost:3000/api/news/getemploymentCount`)
      ...
      async getnewss () {
        ...
        const res = await this.$http.get(`http://localhost:3000/api/news/getemploymentlist?startData=${startData}&countData=${countData}`)
          console.log('success!')
