...
<router-link to="/Home">首页</router-link> /就业动态
					</div>
          <router-link
  ...
  export default {
    ...
    async getListCount () {
      try {
        const res = await this.$http.get(`http://localhost:3000/api/news/getempnoticeCount`)
      ...
      async getnewss () {
        ...
        const res = await this.$http.get(`http://localhost:3000/api/news/getempnoticelist?startData=${startData}&countData=${countData}`)
          console.log('success!')
