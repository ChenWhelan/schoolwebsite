...
<router-link to="/Home">首页</router-link> /招生工作
					</div>
          <router-link
  ...
  export default {
    ...
    async getListCount () {
      try {
        const res = await this.$http.get(`http://localhost:3000/api/news/getenrollmentCount`)
      ...
      async getnewss () {
        ...
        const res = await this.$http.get(`http://localhost:3000/api/news/getenrollmentlist?startData=${startData}&countData=${countData}`)
          console.log('success!')
