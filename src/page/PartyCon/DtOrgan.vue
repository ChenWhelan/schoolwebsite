...
<router-link to="/Home">首页</router-link> /党团组织
					</div>
          <router-link
  ...
  export default {
    ...
    async getListCount () {
      try {
        const res = await this.$http.get(`http://localhost:3000/api/news/getdtorganCount`)
      ...
      async getnewss () {
        ...
        const res = await this.$http.get(`http://localhost:3000/api/news/getdtorganlist?startData=${startData}&countData=${countData}`)
          console.log('success!')
