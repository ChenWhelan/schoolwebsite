...
<router-link to="/Home">首页</router-link> /党团动态
					</div>
          <router-link
  ...
  export default {
    ...
    async getListCount () {
      try {
        const res = await this.$http.get(`http://localhost:3000/api/news/getdtnewsCount`)
      ...
      async getnewss () {
        ...
        const res = await this.$http.get(`http://localhost:3000/api/news/getdtnewslist?startData=${startData}&countData=${countData}`)
          console.log('success!')
