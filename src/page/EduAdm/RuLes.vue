...
<router-link to="/Home">首页</router-link> /规章制度
					</div>
  ...
  export default {
    ...
    async getListCount () {
      try {
        const res = await this.$http.get(`http://localhost:3000/api/news/getrulesCount`)
      ...
      async getnewss () {
        ...
        const res = await this.$http.get(`http://localhost:3000/api/news/getruleslist?startData=${startData}&countData=${countData}`)
          console.log('success!')
