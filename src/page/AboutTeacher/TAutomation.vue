<template>
    <div>
      <div class="breadcumb">
<router-link to="/Home">首页</router-link> /电气工程及其自动化
					</div>
    <div class="box">
...
<script>
 export default {   
    ...
    async getListCount () {
      try {
        const res = await this.$http.get(`http://localhost:3000/api/news/getautomationCount`)
      ...
      async getteacher () {
        ...
        const res = await this.$http.get(`http://localhost:3000/api/news/getautomation?startData=${startData}&countData=${countData}`)
          console.log('success!')
