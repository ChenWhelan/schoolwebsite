<template>
    <div>
      <div class="breadcumb">
<router-link to="/Home">首页</router-link> /智能科学与技术
					</div>
    <div class="box">
...
<script>
 export default {   
    ...
    async getListCount () {
      try {
        const res = await this.$http.get(`http://localhost:3000/api/news/getaiCount`)
      ...
      async getteacher () {
        ...
        const res = await this.$http.get(`http://localhost:3000/api/news/getai?startData=${startData}&countData=${countData}`)
          console.log('success!')
