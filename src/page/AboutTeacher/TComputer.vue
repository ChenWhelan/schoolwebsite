<template>
    <div>
      <div class="breadcumb">
<router-link to="/Home">首页</router-link> /计算机系
					</div>
    <div class="box">
...
<script>
 export default {   
    ...
    async getListCount () {
      try {
        const res = await this.$http.get(`http://localhost:3000/api/news/getcomputerCount`)
      ...
      async getteacher () {
        ...
        const res = await this.$http.get(`http://localhost:3000/api/news/getcomputer?startData=${startData}&countData=${countData}`)
          console.log('success!')
