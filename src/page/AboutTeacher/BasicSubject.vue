<template>
    <div>
      <div class="breadcumb">
<router-link to="/Home">首页</router-link> /基础学部
					</div>
    <div class="box">
...
<script>
 export default {   
    ...
    async getListCount () {
      try {
        const res = await this.$http.get(`http://localhost:3000/api/news/getbasicsubjectCount`)
      ...
      async getteacher () {
        ...
        const res = await this.$http.get(`http://localhost:3000/api/news/getbasicsubject?startData=${startData}&countData=${countData}`)
          console.log('success!')
