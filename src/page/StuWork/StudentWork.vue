...
<router-link to="/Home">首页</router-link> /学工动态
					</div>
      <div >
  ...
  export default {
    ...
    async getListCount () {
      try {
        const res = await this.$http.get(`http://localhost:3000/api/news/getstuworkCount`)
      ...
      async getnewss () {
        ...
        const res = await this.$http.get(`http://localhost:3000/api/news/getstuworklist?startData=${startData}&countData=${countData}`)
          console.log('success!')
