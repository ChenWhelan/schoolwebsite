...
<router-link to="/Home">首页</router-link> /校友之窗
					</div>
      <div >
  ...
  export default {
    ...
    async getListCount () {
      try {
        const res = await this.$http.get(`http://localhost:3000/api/news/getalumniCount`)
      ...
      async getnewss () {
        ...
        const res = await this.$http.get(`http://localhost:3000/api/news/getalumnilist?startData=${startData}&countData=${countData}`)
          console.log('success!')
