...
<router-link to="/Home">首页</router-link> /精品活动
					</div>
      <div >
  ...
  export default {
    ...
    async getListCount () {
      try {
        const res = await this.$http.get(`http://localhost:3000/api/news/getactivityCount`)
      ...
      async getnewss () {
        ...
        const res = await this.$http.get(`http://localhost:3000/api/news/getactivitylist?startData=${startData}&countData=${countData}`)
          console.log('success!')
