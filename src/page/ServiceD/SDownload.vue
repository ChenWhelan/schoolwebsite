...
<router-link to="/Home">首页</router-link> /资料下载/学生专区
					</div>
          <router-link
  ...
  export default {
    ...
    async getListCount () {
      try {
        const res = await this.$http.get(`http://localhost:3000/api/news/getsdownloadCount`)
      ...
      async getnewss () {
        ...
        const res = await this.$http.get(`http://localhost:3000/api/news/getsdownloadlist?startData=${startData}&countData=${countData}`)
          console.log('success!')
